...
from epistemic_mcts.emcts import EpistemicMCTS
...
def run_single_experiment(
    ...
    tag = (
        ...
        "emcts" if not use_llm else
        f"llm_d{drop_rl_n}")
    name_prefix = f"{instruments}_{pool_capacity}_{seed}_{timestamp}_{tag}"
    save_path = os.path.join("./out/e_miner", name_prefix)
    ...
    if resume:
        ...
        all_subdirs = os.listdir('./out/e_miner')
        latest_subdir = all_subdirs[-1]
        list_of_files = glob.glob(f'./out/e_miner/{latest_subdir}/*.pt')

        latest_file = max(list_of_files, key=os.path.getctime)
        latest_step = int(latest_file.split("\\")[-1].split("_")[0])

        list_of_pools = glob.glob(f'./out/e_miner/{latest_subdir}/*.json')
        ...
        model = EpistemicMCTS.load(checkpoint_path, env, policy_net_kwargs)
        model.learn(
        ...
        model = EpistemicMCTS(
            ...
            n_sims=20,   # number of MCTS simulations per planning step
            c=1.41,
            beta=0.5,           # weight on epistemic uncertainty bonus
            gamma=0.99,         # rollout discount factor
            replay_size=10000,   # smaller replay size for demo purposes
            batch_size=128,
            tensorboard_log="./out/e_miner_tensorboard",
        )
